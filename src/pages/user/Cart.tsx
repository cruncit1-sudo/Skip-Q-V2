...
const Cart = () => {
  ...
  // Preload the Razorpay checkout SDK the moment the user enters the cart so
  // that by the time they tap Pay on the next screen the script is already
  // parsed and ready (eliminates the 10-15s click-time download).
  useEffect(() => {
    if (typeof window === "undefined") return;
    if ((window as unknown as { Razorpay?: unknown }).Razorpay) return;
    if (document.querySelector('script[data-razorpay-checkout="true"]')) return;
